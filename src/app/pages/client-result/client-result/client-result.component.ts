...
import { AddPcrFormModel, BindDataModel, TaskBidMasterModel } from "./client-result-model";
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { EncrDecrService } from 'src/app/services/util/encr-decr.service';
import { formsMasterModel, formsQuestionAnswersModel } from '../../admin/pcr-form/forms-master.model';
import { FormsMasterServices } from "../../admin/pcr-form/forms-master.service";
import _ from 'underscore';
import { PCRGrassInfoModel, PCRPropertyInfoModel, PCR_ApplianceModel, PCR_ConveyanceModel, PCR_Damage_MasterModel, PCR_DebrisModel, PCR_FiveBrotherModel, PCR_PoolModel, PCR_RoofModel, PCR_Securing_Model, PCR_UtilitiesModel, PCR_Violation_Model, PCR_WinterizationModel, PCR_Yard_MaintenanceModel } from "../client-result-pcr/client-result-pcr-model";
...
export class ClientResultComponent implements OnInit {
  ...
  workorderId: number;
  selectedIPLNO:any
  questionAnswers: Array<formsQuestionAnswersModel> = [];

  IsReadOnly: boolean;
  AllquestionList: any[] = [];
  initQuestionList: any[] = [];
  //questionAnswers: any[] = [];
  ...
  public drpFormList: Array<string>;
  drpFormArray = [];
  AddPcrFormModelObj: AddPcrFormModel = new AddPcrFormModel();
  GeneratePdfVersion: AddPcrFormModel = new AddPcrFormModel();
  public defaultFormItem: { FormName: string, FormId: number } = { FormName: 'Select', FormId: 0 };
  btnAddPcr = "Save"; // buttom loading..
  isLoadingPcr = false; // buttom loading..
  isFormInValid = false;
  today = new Date();
  userName: any;
  decuser: any;
  hidePcrBtn: boolean = false;
  btnReAdjustPcr = "Re-Adjust Form"; // buttom loading..
  isLoadingReAdjust = false; // buttom loading..
  ...
  constructor(
    ...
    private xRoute: ActivatedRoute, private modalService: NgbModal,
    private spinner: NgxSpinnerService
  ) {
    this.workorder1 = this.xRoute.snapshot.params['workorder'];
    this.routePage = this.xRouter.url;

    xClientResultServices.workorder = this.workorder1;
    this.workorderId = Number(this.EncrDecr.get('123456$#@$^@1ERF', atob(this.workorder1)));

    if (this.xRouter.url === '/client/clientresultinstruction/' + this.workorder1 || this.xRouter.url === '/client/clientresultfield/' + this.workorder1) {
    ...
    if (localStorage.getItem('usertemp_') != null) {
      ...
      if (this.decuser.length > 0) {
        this.userName = " - " + this.decuser[0].User_FirstName;
        this.hidePcrBtn = this.decuser[0].GroupRoleId == 1 || this.decuser[0].GroupRoleId == 2 ? false : true;
      }
    }
  }
...
s1:any
  ...
  getFormsMasterData() {
    ...
    if (this.routePage.includes('client/clientresultfield/')) {
      this.resultType = 2;  //4 to get field result forms
    }
    else if ('client/clientresult/') {
      this.resultType = 1; //3 to get office/client result forms
    }

    this.xClientResultServices.GetformsMaster(this.resultType, workorder).subscribe(res => {
    
      if (res[0].length > 0)
      {
        this.gridData = res[0];
        this.drpFormArray = res[1];
        this.drpFormList = res[1];
        this.noData = false;
      }
      else
      {
  ...
  async DeleteForm(formId) {
    ...
    if (Cnfm) {
      this.xClientResultServices.DeleteForm(formId).subscribe(res => {
        this.getFormsMasterAfterDeleteData();
      });
    }
  }
  getFormsMasterAfterDeleteData() {
    const workorder1 = this.xRoute.snapshot.params['workorder'];
    let workOrderID = this.EncrDecr.get('123456$#@$^@1ERF', atob(workorder1));
    const workorder = parseInt(workOrderID);

    if (this.routePage.includes('client/clientresultfield/')) {
      this.resultType = 4;  //4 to get field result forms
    }
    else if ('client/clientresult/') {
      this.resultType = 3; //3 to get office/client result forms
    }
  ...
  async requiredChange(event: any, FormId: number) {
    //debugger
    this.formsMasterModelObject;
    this.formsMasterModelObject.FormId = FormId;
    this.formsMasterModelObject.Type = 3;

    this.xClientResultServices.PostUpdateFormStatus(FormId, 3)
      .subscribe(res => { });
  }
  ...
  pcrform(content) {
    //debugger
    if (this.routePage.includes('client/clientresultfield/')) {
      localStorage.setItem("ActiveWoTab", "1");  //field result
    }
    else if ('client/clientresult/') {
      localStorage.setItem("ActiveWoTab", "2"); // office result
    }
    this.modalService.open(content, { windowClass: 'sm-model' });
    //this.xRouter.navigate(["/client/clientresultpcr/" + this.workorder1]);
  }

  pcrjson() {
    this.xRouter.navigate(["/client/clientresultpcrjson/" + this.workorder1]);
    //this.xRouter.navigate(["/client/clientresultpcrjson/" + btoa( this.workorder1)]);
  }
  EditForm(IsReadOnlyMode, content, formId) {
    debugger
    this.IsReadOnly = IsReadOnlyMode;
    ...
    this.modalService.open(content, { windowClass: 'lg-model' });
  }
  ...
  async loadFormData(formId) {
    ...
    //set form name
    this.spinner.show();
    this.formDetails = this.gridData.filter(form => form.FormId === formId);
    this.pdfname = this.formDetails[0].FormName + ".pdf";
    if (this.routePage.includes('client/clientresultfield/')) {
      this.resultType = 4;  //4 to get field result forms
    }
    else if ('client/clientresult/') {
    ...
    await this.formsMasterServices.GetformsPreviewData(this.resultType, formId, this.workorderId, this.BindData.address1).subscribe(res => {
      //debugger
      if (res.length > 0) {
        //binding form data
        this.AllquestionList = res[0] && res[0].length > 0 ? res[0] : [];
        if (this.AllquestionList.length) {
          var selectedPList = _.where(this.AllquestionList, { Que_ParentId: 0 });
          if (selectedPList.length > 0) {
            this.questionList = selectedPList;

            var selectedInitList = _.where(this.AllquestionList, { Que_ParentId: 0 });
            this.initQuestionList = selectedInitList;

            this.initQuestionList.forEach(ch => {
              if (ch.QuestionAnswers.length > 0) { // Child Level 1
                var Child1List = _.where(this.AllquestionList, { Que_ParentId: ch.QuestionId });
                let index = this.questionList.findIndex(el => el.QuestionId == ch.QuestionId);
                if (ch.QuestionTypeId == 1) {
                  ch.QuestionAnswers.forEach(chans => {
                    if (chans.AnswerValue == "1") {
                      this.ApplyActionRule(chans.OptionId, ch, index);
                    }
                  });
                }
                else {
                  this.ApplyActionRule(ch.QuestionAnswers[0].AnswerValue, ch, index);
                }

                if (Child1List.length > 0) {
                  Child1List.forEach(ch1 => {
                    if (ch1.QuestionAnswers.length > 0) { // Child Level 2
                      var Child2List = _.where(this.AllquestionList, { Que_ParentId: ch1.QuestionId });
                      let index = this.questionList.findIndex(el => el.QuestionId == ch1.QuestionId);

                      if (ch1.QuestionTypeId == 1) {
                        ch1.QuestionAnswers.forEach(chans => {
                          if (chans.AnswerValue == "1") {
                            this.ApplyActionRule(chans.OptionId, ch1, index);
                          }
                        });
                      } else {
                        this.ApplyActionRule(ch1.QuestionAnswers[0].AnswerValue, ch1, index);
                      }


                      if (Child2List.length > 0) {
                        Child2List.forEach(ch2 => {
                          if (ch2.QuestionAnswers.length > 0) { // Child Level 3
                            var Child3List = _.where(this.AllquestionList, { Que_ParentId: ch2.QuestionId });
                            let index = this.questionList.findIndex(el => el.QuestionId == ch2.QuestionId);

                            if (ch2.QuestionTypeId == 1) {
                              ch2.QuestionAnswers.forEach(chans => {
                                if (chans.AnswerValue == "1") {
                                  this.ApplyActionRule(chans.OptionId, ch2, index);
                                }
                              });
                            } else {
                              this.ApplyActionRule(ch2.QuestionAnswers[0].AnswerValue, ch2, index);
                            }
                            if (Child3List.length > 0) {
                              Child3List.forEach(ch3 => {
                                if (ch3.QuestionAnswers.length > 0) { // Child Level 4
                                  let index = this.questionList.findIndex(el => el.QuestionId == ch3.QuestionId);
                                  if (ch3.QuestionTypeId == 1) {
                                    ch3.QuestionAnswers.forEach(chans => {
                                      if (chans.AnswerValue == "1") {
                                        this.ApplyActionRule(chans.OptionId, ch3, index);
                                      }
                                    });
                                  } else {
                                    this.ApplyActionRule(ch3.QuestionAnswers[0].AnswerValue, ch3, index);
                                  }
                                }
                              });
                            }
  ...
  async ddlChange(ev, que, idx) {
    ...
    if (que.InvalidFR == false) {
      if (ev.target.value != null) {
        let model = new formsQuestionAnswersModel();
        ...
        if (ev.target.value != null) {
          let model = new formsQuestionAnswersModel();
          model.FormId = this.formId;
          model.QuestionId = que.QuestionId;
          model.AnswerValue = ev.target.value; //ev.target.options[ev.target.options.selectedIndex].text;
          model.QuestionTypeId = que.QuestionTypeId;
          model.WorkOrderId = this.workorderId;
          model.Address = this.BindData.address1;

          model.OptionId = ev.target.value;

          this.questionAnswers.push(model);

          this.formsMasterServices.PostQuestionAnswer(this.questionAnswers).subscribe(res => {
            if (res.length > 0) {
              let index = this.questionList.indexOf(que);
              this.questionList[index].QuestionAnswers = this.questionAnswers;
            }
          });
        }

        this.selectedItem = new Object();
        this.selectedItem = this.questionList.find((x: any) => x.QuestionId == que.QuestionId).ActionRulesMaster;
        //this.selectedItem = this.questionList;

        this.selectedItem.QuestionId = que.QuestionId;
        this.selectedQuestion = [];
        this.selectedItem.forEach(doc => {
          let thingsObj = {} as any;
          if (doc.ActionRule_Value == ev.target.value) {
            doc.ActionRules.forEach(element => {
              if (element.ActionValue === 'Show' && doc.ActionRule_Operator === 'Equals') {

                thingsObj.ActionRule_Operator = doc.ActionRule_Operator;
                thingsObj.ActionRule_Value = doc.ActionRule_Value;
                thingsObj.ActionQuestionId = doc.ActionQuestionId;
                thingsObj.ActionRules = doc.ActionRules;
                //  thingsObj.Question = doc.Question;
                thingsObj.QuestionId = doc.QuestionId;
                thingsObj.QuestionTypeId = doc.QuestionTypeId;
                this.selectedQuestion.push(thingsObj);

              }
              else if (element.ActionValue === 'Show' && doc.ActionRule_Operator === 'Not Equals') {
                thingsObj.ActionRule_Operator = doc.ActionRule_Operator;
                thingsObj.ActionRule_Value = doc.ActionRule_Value;
                thingsObj.ActionQuestionId = doc.ActionQuestionId;
                thingsObj.ActionRules = doc.ActionRules;
  ...
  async textChange(ev, que, idx) {
    ...
    if (ev.target.value != null) {
      ...
      if (que.InvalidFR == false) {
        ...
        model.FormId = this.formId;
        model.QuestionId = que.QuestionId;
        model.AnswerValue = ev.target.value;
        model.QuestionTypeId = que.QuestionTypeId;
        model.WorkOrderId = this.workorderId;
        model.Address = this.BindData.address1;
  ...
  async checkChange(ev, option, que, idx) {
    //debugger
    //console.log(que);
    var optionId = option.OptionId;

    let index = que.Options.findIndex(el => el.OptionId == option.OptionId);
    que.Options[index].AnswerValue = que.Options[index].AnswerValue == "0" ? "1" : "0";
    ...
    if (que.InvalidFR == false) {
      this.questionAnswers = [];
      //console.log('checked ev', ev.target.checked);
      if (ev.target.value) {

        que.Options.forEach(element => {
          let model = new formsQuestionAnswersModel();
          model.FormId = this.formId;
          model.QuestionId = que.QuestionId;
          model.AnswerValue = element.AnswerValue;
          model.QuestionTypeId = que.QuestionTypeId;
          model.WorkOrderId = this.workorderId;
          model.Address = this.BindData.address1;
          model.OptionId = element.OptionId;
        ...
        this.formsMasterServices.PostQuestionAnswer(this.questionAnswers).subscribe
          (
            res => {
              if (res.length > 0) {
                let index = this.questionList.indexOf(que);
  ...
  async radioQueChange(ev, optionId, que, idx) {
    ...
    if (ev.target.value && que.InvalidFR == false) {
      let model = new formsQuestionAnswersModel();
      model.FormId = this.formId;
      model.QuestionId = que.QuestionId;
      model.AnswerValue = ev.target.value;
  ...
  async copyAnswer(ev, questionId: number) {
  }
  ...
  EditGrassForm(content) {
    this.GetGrassDetail();
    this.modalService.open(content, { windowClass: 'lg-model' });
  }
  EditPrsvForm(content) {
    this.GetPropertyInfoData();
    this.modalService.open(content, { windowClass: 'lg-model' });
  }
  OpenPcrFormModel(content,formData,type){

    this.selectedIPLNO = this.ModelObj.IPLNO
    // console.log('sandip454646r',this.selectedIPLNO)
     debugger;
    if(type=0)
    {
      this.pcrFormObject=formData;
    ...
    {
      var obj={
        Fb_Dynamic_Tab_Name:formData.FormName,
        Fb_Dynamic_pkeyID:formData.FormId,
        FWO_PkyeId:formData.fwo_pkyeId
      }
      this.pcrFormObject=obj;
      this.modalService.open(content, { windowClass: 'xlModal' });
    }
  ...
  FormFilter(value) {
    ...
    var filteredcom = this.drpFormArray.filter(function (el) {
      return el.FormName != null;
    });
    if (value != '') {
      this.drpFormList = filteredcom.filter((s) => s.FormName.toLowerCase().indexOf(value.toLowerCase()) !== -1);
    }
    else {
      this.drpFormList = this.drpFormArray.slice();
    }
  }

  PcrSubmit() { 
    if (this.AddPcrFormModelObj.Pcr_FormId > 0) {


      var filteredcom = this.drpFormArray.filter(function (el) {
        return el.FormId != 0;
      });
      var getselectedForm=filteredcom.filter((x)=>x.FormId ===this.AddPcrFormModelObj.Pcr_FormId);
      this.isLoadingPcr = true;
      this.btnAddPcr = "Processing";
      this.isFormInValid = false;
      this.AddPcrFormModelObj.Pcr_FormWoId = this.workorderId;
      this.AddPcrFormModelObj.Type = 1;
      this.AddPcrFormModelObj.fwo_IsPcrAdd = true;this.AddPcrFormModelObj.fwo_IsPcrAdd = true;
      if(getselectedForm.length==0)
      {
        this.AddPcrFormModelObj.IsDynamicForm=false;
      }
      else
      {
        this.AddPcrFormModelObj.IsDynamicForm=getselectedForm[0].IsDynamicForm
      }

      if (this.routePage.includes('client/clientresultfield/')) {
        this.AddPcrFormModelObj.Fwo_IsFieldResult = true;  //field result
      }
      else if ('client/clientresult/') {
        this.AddPcrFormModelObj.Fwo_IsOfficeResult = true; // office result
      }

      this.formsMasterServices.AddPcrForm(this.AddPcrFormModelObj)
        .subscribe(response => {
          this.isLoadingPcr = false;
          this.btnAddPcr = "Save";
          if (response[0].length != 0) {
            this.MessageFlag = "Pcr form added...!";
            this.commonMessageClose();
            this.AddPcrFormModelObj = new AddPcrFormModel();
            this.getFormsMasterData();
          }
        });
    }
  ...
  ReAdjustPcrForm() {
    this.isLoadingReAdjust = true;
    this.btnReAdjustPcr = "Processing";
    this.AddPcrFormModelObj.Pcr_FormWoId = this.workorderId;
    this.AddPcrFormModelObj.Type = 6;

    this.formsMasterServices.AddPcrForm(this.AddPcrFormModelObj)
      .subscribe(response => {
        this.isLoadingReAdjust = false;
        this.btnReAdjustPcr = "Re-Adjust Form";
        //debugger;
        if (response[1] == "1") {
          this.MessageFlag = "Re-adjustment form added...!";
          this.commonMessage();
          this.getFormsMasterData();
        }
        else{
          this.MessageFlag = "Add form for Re-adjustment...!";
          this.commonMessage();
        }
      });
  }
  ...
  IsPDFVersionDownload=false
  pdfDownloadUrl="";
  FormToPDFVersion(pdfDetails){
    // console.log(pdfDetails);
    this.GeneratePdfVersion.Pcr_FormWoId = this.workorderId;
    this.GeneratePdfVersion.Pcr_FormId = pdfDetails[0].FormId;
    this.GeneratePdfVersion.Type = 1;

    this.formsMasterServices.GeneratePdfVersionDocument(this.GeneratePdfVersion)
      .subscribe(response => {
        this.pdfDownloadUrl=response[0]
        this.IsPDFVersionDownload=true;
      });
  }
}
