...
export class PhotoGalleryWithEditor implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.labels != undefined) {
      this.labels.forEach((item) => {
        Object.keys(item).forEach(key => {
          if (key.includes('ButtonName') && item[key] !== "" && item[key] !== null) {
            let obj = {};
            obj["name"] = item.Task_Label_Name;
            obj["buttonName"] = item[key];
            this.photoLabels.push(obj);
          }
        });
      })
    }
    ...
    this.canvas.addEventListener('mouseup', (e) => {
      if (this.isDragging === true) {
        ...
        if(this.editType===0)
        {
          this.CropImage(this.startX, this.startY,Math.abs(e.offsetX - this.startX), Math.abs(e.offsetY - this.startY));
        }
        else
        {
          let image = new Image();
  ...
  imageLoad() {
    ...
    //console.log('check metadata', this.metadata)
    this.photo = this.metadata.Client_Result_Photo_FilePath;
  ...
  scaleToFit(ctx, img) {
    ...
    ctx.drawImage(whiteImage, 0, 0, this.canvas.width, this.canvas.height);
  ...
  CropImage(sourceX,sourceY,sourceWidth,sourceHeight) {
    const ctx = this.canvas.getContext('2d');
    ctx.imageSmoothingEnabled = false;
    var destWidth = this.canvas.width;
    var destHeight = this.canvas.height;
    var destX = ctx.canvas.width / 2 - destWidth / 2;
    var destY = ctx.canvas.height / 2 - destHeight / 2;
    ctx.save();
    ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    ctx.translate(this.canvas.width / 2, this.canvas.height / 2);
    ctx.scale(this.scale, this.scale);
    ctx.translate(-this.canvas.width / 2, -this.canvas.height / 2);
    ctx.drawImage(this.currentImage, sourceX, sourceY, sourceWidth, sourceHeight, destX, destY, destWidth, destHeight);
    this.editType = 0;
  }
  ...
  changeLabel(e) {
    this.photoLabel = e.target.value;
  }
