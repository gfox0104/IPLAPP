...
import { TmpFbFormModel } from '../../pcr-form/forms-master.model';
import { FormsMasterServices } from '../../pcr-form/forms-master.service';


@Component({
  selector: 'app-service-link-template',
  templateUrl: './service-link-template.component.html',
  styleUrls: ['./service-link-template.component.scss']
})
export class ServiceLinkTemplateComponent implements OnInit {
  gridData: any[];
  ...
  async GetFBFormTemplateData() {
    // debugger
    await this.formsMasterServices.GetAdminFormList(1,12).subscribe(res => {
      if (res[0].length > 0) {
        this.state.take = 15;
        this.state.skip = 0;
        this.gridData = res[0];
  ...
  wtFormButton() {
    ...
    this.gridData.forEach(element => {
      ...
      this.tmpFbFormModelSaveObj.Type = 2;
