...
import { ImportQueueServices } from '../../../work-order/import-queue-order/import-queue-order.service';
type AOA = any[][];
...
@Component({
  selector: 'app-import-client-payment-order',
  templateUrl: './import-client-payment-order.component.html',
})
export class ImportClientPaymentOrderComponent implements OnInit {
  ...
  onFileChange(evt: any) {
    ...
    /* wire up file reader */
    if (this.fileFormat === '.xlsx' || this.fileFormat === '.xls') {
...
async ExcelDataFile() {
  ...
  try {
    ...
    if (this.inputelement == '' || this.inputelement == undefined)
    {
      ...
      if (extsn != this.fileFormat ) {
        errCnt = errCnt + 1;
        this.MessageFlag = this.fileFormat === '.xlsx' || '.xls' ? "please select excel file ...!" : "please select pdf file ...!" ;
        this.commonMessage();
    ...
    if(errCnt > 0)
    {
      ...
      this.spinner.hide();
      this.xRoute.navigate(["accounting/accountingdetails/exceldata"]);
  ...
  onSelectChange(event) {
    ...
    // this.fileFormat = this.fileTypeList.find(item => item.type === event.target.value).ext1;
  }
