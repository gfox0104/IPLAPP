...
@Component({
  selector: 'app-nfr-template-fb-forms',
  templateUrl: './nfr-template-fb-forms.component.html',
  styleUrls: ['./nfr-template-fb-forms.component.scss']
})
export class NFRTemplateFbFormsComponent implements OnInit {
  ...
  async GetFBFormTemplateData() {
    await this.formsMasterServices.GetAdminFormList(1,7).subscribe(res => {
      if (res[0].length > 0) {
        this.state.take = 15;
        this.state.skip = 0;
        this.gridData = res[0];
