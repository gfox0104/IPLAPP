...
@Component({
  selector: 'app-mcs-template-fb-forms',
  templateUrl: './mcs-template-fb-forms.component.html',
  styleUrls: ['./mcs-template-fb-forms.component.scss']
})
export class MCSTemplateFbFormsComponent implements OnInit {
  gridData: any[];
  ...
  async GetFBFormTemplateData() {
    await this.formsMasterServices.GetAdminFormList(1,6).subscribe(res => {
      if (res[0].length > 0) {
        this.state.take = 15;
        this.state.skip = 0;
        this.gridData = res[0];
