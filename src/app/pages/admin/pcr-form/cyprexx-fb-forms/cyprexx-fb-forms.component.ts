...
@Component({
  selector: 'app-cyprexx-fb-forms',
  templateUrl: './cyprexx-fb-forms.component.html',
  styleUrls: ['./cyprexx-fb-forms.component.scss']
})
export class CyprexxFbFormsComponent implements OnInit {
  ...
  async GetFBFormTemplateData() {
    // debugger
    await this.formsMasterServices.GetAdminFormList(1,10).subscribe(res => {
      if (res[0].length > 0) {
        this.state.take = 15;
        this.state.skip = 0;
        this.gridData = res[0];
