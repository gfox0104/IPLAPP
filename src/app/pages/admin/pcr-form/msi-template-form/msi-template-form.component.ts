...
@Component({
  selector: 'app-msi-template-form',
  templateUrl: './msi-template-form.component.html',
  styleUrls: ['./msi-template-form.component.scss']
})
export class MsiTemplateFormComponent implements OnInit {
  ...
  async GetFBFormTemplateData() {
    // debugger
    await this.formsMasterServices.GetAdminFormList(1,3).subscribe(res => {
      if (res[0].length > 0) {
        this.state.take = 15;
        this.state.skip = 0;
        this.gridData = res[0];
